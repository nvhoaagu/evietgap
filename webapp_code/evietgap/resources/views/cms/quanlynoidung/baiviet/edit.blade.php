...
<div class="panel panel-primary">
  <div class="panel-heading">
    <h3 class="panel-title">Chỉnh sửa bài viết</h3>
  </div>
  ...
  @endif
    <form action="" method="POST" role="form">
      <div class="form-group">
        ...
        <input type="text" class="form-control" name="TenBaiViet" value="{{ $baiviet->TenBaiViet }}" placeholder="Tên bài viết">
      </div>
      <div class="form-group">
        <label for="">Nội Dung Bài Viết</label>
        <textarea name="NoiDungBaiViet" placeholder="Nội dung" id="textarea" class="form-control">
      ...
      <div class="form-group">
        ...
        <select name="MaTheLoai" id="inputMaTheLoai" class="form-control" required="required">
          ...
          @if($tl->TheLoaiCha == 0)
           <option value="{{$tl->MaTheLoai}}" @if($tl->MaTheLoai == $baiviet->MaTheLoai)selected="selected"@endif>{{$tl->TenTheLoai}}</option>
            @foreach($theloai as $t)
              @if($t->TheLoaiCha == $tl->MaTheLoai)
                <option value="{{$t->MaTheLoai}}" @if($t->MaTheLoai == $baiviet->MaTheLoai)selected="selected"@endif>--{{$t->TenTheLoai}}</option>
              @endif
            @endforeach
          @endif
          @endforeach
...
$(function(){
  var i = 1;
    ...
    $("*[name='slide']").prop("checked", '<?php echo $baiviet->slide?true:false; ?>');
