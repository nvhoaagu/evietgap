  ...
  @endif
    <form action="" method="POST" role="form">
      <div class="form-group">
        <label for="">Tiều Đề Trang (Hiển thị ngoài trang chủ)</label>
        <input type="text" class="form-control" name="TitlePage" placeholder="Tiêu đề trang" value="{{$baiviet->TitlePage}}">
      </div>
      <div class="form-group">
        <label for="">Tên đầy đủ của trang </label>
        <input type="text" class="form-control" name="TenPage" placeholder="Tên trang" value="{{$baiviet->TenPage}}">
      </div>
      <div class="form-group">
        <label for="">Nội Dung</label>
        <textarea name="NoiDung" placeholder="Nội dung" id="textarea" class="form-control">{!!$baiviet->NoiDung!!}</textarea>
        <script>
      ...
      <div class="form-group">
        <div class="input-group" data-toggle="modal" data-target="#myModal">
          ...
          <input type="hidden" class="form-control" id="idDK" name="MaFile" value="{{ $baiviet->MaFile}}">
        </div>
      </div>
      <div class="form-group">
        <label for="">Thể Loại</label>
        <?php $theloai = DB::table("cms_theloai_page")->where("TrangThai",1)->get(); ?>
        <select name="LoaiPage" id="inputMaTheLoai" class="form-control" required="required">
          @foreach($theloai as $tl)
           <option value="{{$tl->id}}" @if($tl->id == $baiviet->LoaiPage)selected="selected"@endif>{{$tl->TenTheLoai}}</option>
          @endforeach
        </select>
      </div>
      {{ csrf_field() }}
      <button type="submit" class="btn btn-primary">Đăng Bài</button>
      <a href="{{ url('quantrinoidung/page') }}" class="btn btn-default">Thoát</a>
    </form>
...
$(function(){
  var i = 1;
    ...
    var text = '';
    var check = "{{$baiviet->MaFile}}";
    check = check.split(',');
    var na = "";
    $.each(list, function(index, val) {
      if(check.indexOf(''+val.MaFile+'') > -1){
        var ch = 'checked="checked"';
        na += val.TenFile+",";
        $("#nameDK").val(na);
      }
      text += '<div class="checkbox"><label><input type="checkbox" '+ch+' value="'+val.MaFile+'" class="check">'+val.TenFile+'</label></div>';
    });
    $("#dkselect").html(text);
