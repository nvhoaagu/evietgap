...
$(document).ready(function(){
  $('#hangdoisinhvienangiang').DataTable({
  });
  if($('table').get() == true){
    if( $('tfoot').get() == false)
    {
      var tfoot = $('.table thead').clone().html();
      tfoot.replace('thead','tfoot');
      $('.table tbody').after("<tfoot>"+tfoot+"</tfoot>");
    }

    $('.table tfoot tr *').each( function () {
          var title = $('.table thead tr *').eq( $(this).index() ).text();
